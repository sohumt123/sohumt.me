...
class CanvAscii {
  ...
  render() {
    ...
    // Animate ASCII font size between 6 and 20
    const animatedFontSize = 13 + 7 * Math.sin(time * 0.5);
    this.filter.fontSize = Math.max(6, Math.min(20, animatedFontSize));
    this.filter.reset();
  ...
  clear() {
    this.scene.traverse(obj => {
      if ((obj as THREE.Mesh).isMesh && typeof (obj as THREE.Mesh).material === 'object' && (obj as THREE.Mesh).material !== null) {
        const material = (obj as THREE.Mesh).material as THREE.Material & Record<string, unknown>;
        Object.keys(material).forEach(key => {
          const matProp = material[key];
          if (matProp !== null && typeof matProp === 'object' && 'dispose' in matProp && typeof (matProp as { dispose: unknown }).dispose === 'function') {
